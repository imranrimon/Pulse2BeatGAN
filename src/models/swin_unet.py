...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., pretrained_window_size=0):
        ...
        coords_l = torch.arange(self.window_size) # Wl
        ...
        self.register_buffer("relative_position_index",relative_position_index) # Wl, Wl
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=8, shift_size=0,
        ...
        self.attn = WindowAttention(
            ...
            qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, pretrained_window_size=pretrained_window_size)
    ...
    def forward(self, x):
        ...
        # # padding x to multiple window size
        # pad_r = (self.window_size - L % self.window_size) % self.window_size
        # x = F.pad(x, (0, 0, 0, pad_r)) # pad to multiple window size
        # _, Lp, _ = x.shape
        ...
        shifted_x = window_reverse(attn_windows, self.window_size, H)  # B H' C
...
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.expand = nn.Linear(dim, dim_scale * dim, bias=False) if self.dim_scale == 2 else nn.Identity() 
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, C)
        x = rearrange(x, 'b h (p1 c)-> b (h p1) c', p1=self.dim_scale , c=C // self.dim_scale )
...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=4, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        # print("FinalPatchExpand_X4 expand initial----input_resolution:{};dim:{};dim_scale:{}".format(input_resolution, dim, dim_scale))
        self.dim = dim
        self.dim_scale = dim_scale
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, C)
        x = rearrange(x, 'b h  (p1 c)-> b (h p1)  c', p1=self.dim_scale,
                      c=C // (self.dim_scale ))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size=4, in_chans=32, embed_dim=128, norm_layer=None):
        super().__init__()

        # patches_resolution = [img_size // patch_size]
        self.img_size = img_size
        self.patch_size = patch_size
        self.num_patches = math.ceil(img_size / patch_size)
        self.patches_resolution = math.ceil(img_size / patch_size)
    ...
    def forward(self, x):
        ...
        assert H == self.img_size , f"Input image size ({H} doesn't match model ({self.img_size})."
        x = self.proj(x).flatten(2).transpose(1, 2) # (1,128,96)
        if self.norm is not None:
            x = self.norm(x) # (1,128,96)
        return x

    def flops(self):
        Ho = self.patches_resolution
        flops = Ho * self.embed_dim * self.in_chans * self.patch_size
        if self.norm is not None:
            flops += Ho * self.embed_dim
        return flops
...
class SwinTransformerSys(nn.Module):
    ...
    class GuidedAttentionBlock(nn.Module):
        ...
        def forward(self, x):
            ...
            # Respath Connection (Multiplication)
            respath_mult = x * attn
            
            # Batch Normalization
            respath_mult = self.batch_norm(respath_mult)
            
            # Respath Connection (Summation)
            out = respath_mult + x
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x)    # x_size: torch.Size([1, 128, 96])
        x_downsample = []
    ...
    def forward_up_features(self, x, x_downsample, norm_up):
        out = []
    ...
    def forward(self, x):
        ...
        x = self.forward_up_features(x, x_downsample, norm_up=[self.norm_up_128, self.norm_up_256, self.norm_up_512])
...
model = SwinTransformerSys(img_size=[128, 128*2, 128*4], 
                           patch_size=4, 
                           in_chans=1, 
                           num_classes=1, 
                           embed_dim=96,
...
x_128, x_256, x_512 = model(torch.randn(1, 1, 128*4))